...
# Set the local directory for the mirror
ENV LOCAL_DIR="/app"
...
CMD ["sh", "-c", "lftp -e \"set xfer:log 1; set cmd:default-protocol sftp; mirror --verbose --log=${LOCAL_DIR}/mirror_$(date | sed s/\\ /\\_/g).log --parallel=$PARALLEL_TRANSFERS --only-newer --delete $REMOTE_DIR ${LOCAL_DIR}/mirror; bye\" $MIRROR_URL"]
